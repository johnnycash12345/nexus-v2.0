...
class NexusQuantumReasoning:
    ...
    def plan_research_4_0(
        ...
        tools_block = "\n".join(f"- {tool}" for tool in available_tools) or "- Nenhuma ferramenta cadastrada."
        system_prompt = (
        ...
        fallback_tool = (
            available_tools[0] if available_tools else "tavily_search"
        )

        normalized = {
            "tool": response_payload.get("tool") or fallback_tool,
            "search_query": response_payload.get("search_query") or query,
            "context_of_use": response_payload.get("context_of_use") or "Pesquisa Geral",
        }
        return normalized
    ...
    def re_rank_by_confidence(self, documents: Sequence[Any]) -> List[Any]:
        ...
        for document in documents:
            intrinsic = self._get_numeric_attr(document, "confianca_intrinseca", default=0.0)
        ...
        if scored_docs:
            best_score = self._get_numeric_attr(scored_docs[0], "score_final", default=0.0)
        ...
        for document in scored_docs:
            intrinsic_value = self._get_numeric_attr(document, "confianca_intrinseca", default=0.0)
            if intrinsic_value <= 0.0:
                continue
            node_id = self._get_attr(document, "id")
            if not node_id:
                continue
            boost = 2.0 if node_id == best_doc_id else 1.0
            validated = node_id == best_doc_id
            try:
                database.register_memory_activation(node_id, boost=boost, validated=validated)
            except Exception as error:  # noqa: BLE001
                print(f"[NQR] Aviso: nao foi possivel reforcar memoria {node_id}: {error}")
        return scored_docs

    # ------------------------------------------------------------------
    # Auto-correcao
    # ------------------------------------------------------------------
    def self_correct_rag(self, final_response: str, context_facts: Sequence[Any]) -> str:
        """
        ...
        high_confidence_docs = [
            ...
            if self._get_numeric_attr(doc, "confianca_intrinseca", default=0.0) >= HIGH_CONFIDENCE_THRESHOLD
        ]
    ...
    def _diagnose_contradiction(self, response: str, document: Any) -> Dict[str, Any]:
        ...
        system_prompt = (
            "Voce e o modulo de Auto-Correcao do Nexus-Quantum-Reasoning.\n"
        ...
        *,
        ...
        try:
            payload = {
                ...
                "fonte_titulo": self._get_attr(document, "title", default=self._get_attr(document, "titulo", default="")),
            }
    ...
    def _run_gemini_completion(
        ...
        *,
        ...
        api_key = settings.ai.google.api_key or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
        ...
        try:
            ...
            print(f"[NQR] Falha na chamada Gemini ({model_name}): {error}. Fallback para DeepSeek.")
            client, model = self._get_deepseek_client()
            completion = client.chat.completions.create(
                model=model,
                messages=[
    ...
    def _pick_provider(self) -> str:
        try:
            ...
            if settings.ai.google.enabled:
                key_available = settings.ai.google.api_key or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
                if key_available and genai is not None:
                    return "gemini"
            if settings.ai.deepseek.enabled and os.getenv("DEEPSEEK_API_KEY"):
                return "deepseek"
        except Exception as error:  # noqa: BLE001
            print(f"[NQR] Aviso: falha ao carregar configuracoes de IA ({error}). Usando DeepSeek.")
        return "deepseek"
    ...
    def _load_settings(self) -> SystemSettings:
        try:
            ...
            print(f"[NQR] Nao foi possivel carregar configuracoes. Usando padrao. Detalhe: {error}")
    ...
    def _get_numeric_attr(self, document: Any, name: str, default: float = 0.0) -> float:
